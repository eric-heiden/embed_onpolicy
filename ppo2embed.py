...
class Runner(object):
    ...
    def __init__(self, *, env: VecNormalize, model: Model, nsteps, gamma, lam):
        self.env = env
        self.model = model
        nenv = env.num_envs
        ...
        self.obs[:] = env.reset()
        self.tasks = [e.env.task for e in env.venv.envs]
    ...
    def run(self):
        ...
        for _ in range(self.nsteps):
            ...
            if "t" in infos[-1]["episode"]:
                # handle malicious sum(rewards) from gym.Monitor
                print("DONE")
                mb_tasks.append(mb_tasks[-1])
            else:
            ...
            if done:
                ...
                self.tasks = [e.env.task for e in self.env.venv.envs]
                self.latents = [self.model.get_latent(t) for t in self.tasks]
                done = False
