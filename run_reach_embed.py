...
sys.path.insert(0, osp.join(osp.dirname(__file__), 'garage_rjzp'))
...
def train(num_timesteps, seed, log_folder):
    ...
    task_space = gym.spaces.Box(low=0, high=1, shape=(len(GOALS),), dtype=np.float32)
    ...
    def plot_traj(fig, where, task, batch_tuple_size, batches, colormap):
        ...
        ax.set_title("Task %i (%s)" % (task, str(GOALS[task])))
        ax.grid()
        ax.set_aspect("equal")
        ax.set_xlabel('x')
        ax.set_ylabel('y')
    ...
    def render_robot(task: int, iteration: int):
        ...
        def render(env: SawyerEnv, obs, actions, values, rewards, infos):
            ...
            draw_left.text((20, 20), "Task %i (%s)" % (task + 1, str(GOALS[task])), fill="black", font=font)
...
def main():
    ...
    log_folder = osp.join(osp.dirname(__file__), 'log/reach_pos_embed_%i_%s' % (SEED, timestamp))
